...
class TestPackageTask:
    ...
    def test__get_birth_application(self, mocker, task):
        mock_request = mocker.MagicMock(spec=VitalRecordsRequest)
        mock_request.id = task.kwargs["request_id"]
        ...
        application = task._get_birth_application(mock_request)

        assert isinstance(application, BirthApplication)
        assert application.package_id == mock_request.id
        assert application.WildfireName == mock_request.fire.capitalize()
        assert application.NumberOfCopies == mock_request.number_of_records
        assert application.RegFirstName == mock_request.first_name
        assert application.RegMiddleName == mock_request.middle_name
        assert application.RegLastName == mock_request.last_name
        ...
        assert application.Parent1FirstName == mock_request.person_1_first_name
        assert application.Parent1LastName == mock_request.person_1_last_name
        assert application.Parent2FirstName == mock_request.person_2_first_name
        assert application.Parent2LastName == mock_request.person_2_last_name
        ...
        assert application.RequestorCity == mock_request.city
        assert application.RequestorStateProvince == mock_request.state
        assert application.RequestorZipCode == mock_request.zip_code
        assert application.RequestorCountry == "United States"
        assert application.RequestorEmail == mock_request.email_address
        assert application.RequestorTelephone == mock_request.phone_number

    def test__get_marriage_application(self, mocker, task):
        mock_request = mocker.MagicMock(spec=VitalRecordsRequest)
        mock_request.id = task.kwargs["request_id"]
        mock_request.fire = "palisades"
        mock_request.number_of_records = 2
        ...
        application = task._get_marriage_application(mock_request)

        assert isinstance(application, MarriageApplication)
        assert application.package_id == mock_request.id
        assert application.WildfireName == mock_request.fire.capitalize()
        assert application.NumberOfCopies == mock_request.number_of_records
        assert application.Spouse1FirstName == mock_request.person_1_first_name
        assert application.Spouse1MiddleName == mock_request.person_1_middle_name
        assert application.Spouse1LastName == mock_request.person_1_last_name
        assert application.Spouse1BirthLastName == mock_request.person_1_birth_last_name
        assert application.Spouse2FirstName == mock_request.person_2_first_name
        assert application.Spouse2MiddleName == mock_request.person_2_middle_name
