...
resource "azurerm_key_vault_secret" "django_allowed_hosts" {
  ...
  value        = format("%s,www.%s,%s", local.hostname, local.hostname, module.application.app_fqdns.web)
...
resource "azurerm_key_vault_secret" "django_trusted_origins" {
  ...
  value        = format("https://%s,https://www.%s,https://%s", local.hostname, local.hostname, module.application.app_fqdns.web)
  key_vault_id = module.key_vault.key_vault_id
  content_type = "text"
}
